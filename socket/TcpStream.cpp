...
TcpStream::TcpStream(int sock, size_t bufSize) :
    ...
    socketTcpDisableNagle(sock);
}

int TcpStream::listen(char addrstr[MAX_ADDRSTR_LEN]) {
    m_sock = socketTcpLoopbackServer(0, SOCK_STREAM);
    if (!valid())
        return int(ERR_INVALID_SOCKET);

    int port = socketGetPort(m_sock);
    if (port < 0) {
        ::close(m_sock);
        return int(ERR_INVALID_SOCKET);
    }
...
SocketStream * TcpStream::accept() {
    int clientSock = socketAccept(m_sock);
    if (clientSock < 0)
        return NULL;
...
int TcpStream::connect(const char* addr) {
    int port = atoi(addr);
    m_sock = socketTcpLoopbackClient(port, SOCK_STREAM);
    return valid() ? 0 : -1;
}

int TcpStream::connect(const char* hostname, unsigned short port)
{
    m_sock = socketTcpClient(hostname, port, SOCK_STREAM);
    return valid() ? 0 : -1;
}
